...
void main()
{
	// if no uniform boolean statments are true, set gl position and out vec2 texture coordiantes variable as normal
	...
	*/
	...
	framebuffer_screen_quad_texture_coordiantes = framebuffer_quad_data.zw; 

	// if chaos uniforrm boolean statement is true, create a chaos strength variable that will store a constant float value for the offset of the texture coordinates
	if (post_processing_effect_chaos)
	{
		...
		// sum of the framebuffer_screen_quad_texture_coordiantes x-coordinate and sin value of post_processing_time (to keep the time value clamped at -1 and 1) multiplied by the strength value
		// same concept applies to the framebuffer_screen_quad_texture_coordiantes's y-coordinate however this time we use the cos value of post_processing_time (to keep the time value clamped at -1 and 1)
		vec2 texture_chaos_coordiante_offset = vec2(framebuffer_screen_quad_texture_coordiantes.x + sin(post_processing_time) * chaos_strength, framebuffer_screen_quad_texture_coordiantes.y + cos(post_processing_time) * chaos_strength);
